...
issueIds = Ids.split(',') as List;
final def failMode        = FailMode.valueOf(props['failMode']);
//final def failMode        = FailMode.WARN_ONLY;
//final def failMode        = FailMode.FAIL_ON_ANY_FAILURE;
//final def customFieldName = "customfield_10112";  //BMC Ref NO: customFieldId=10112
				...
				for (def remoteValue : customFieldValues) {
					if (remoteValue.getCustomfieldId() == customFieldName) {
						...
						if (values.length != 0) {
							...
							try {								
								outProps.setProperty("BMC Ref No", "$values");
